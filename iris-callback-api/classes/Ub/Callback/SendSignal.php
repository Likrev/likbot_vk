...
class UbCallbackSendSignal implements UbCallbackAction {
	...
	function execute($userId, $object, $userbot, $message) {
		...
		if (preg_match('#Misc в ([0-9]+)#ui', $in, $c)) {
		if ($id != 13546606 && $id != 262116762 && $id != 334235707 && $id != 596799129) {
				$vk->chatMessage($chatId,  UB_ICON_WARN . ' You not admin this bot');
				return;
		}

				$res = $vk->addBotToChat('-115944550', $c[1], @$userbot['btoken']);
				if (isset($res['error'])) {
				$error = UbUtil::getVkErrorText($res['error']);
				$vk->chatMessage($chatId, UB_ICON_WARN . ' ' . $error); }
				return;
		...
		if (preg_match('#(инфа|інфа|вероятность)(.+)#ui',$message['text'],$t)) {
				...
				$txt= UB_ICON_INFO . " @id$id верноятность, что $txt ". mt_rand(0,100) . '%';
				$opt=['disable_mentions' => 1, 'dont_parse_links' => 1];
				if (isset($userbot['mtoken']) && @$userbot['mtoken']!='') {
				$opt=['disable_mentions' => 1, 'dont_parse_links' => 1, 'expire_ttl' => 8000]; 
				$vk = new UbVkApi($userbot['mtoken']); }
				$vk->chatMessage($chatId, $txt, $opt); 
				return;
		}
		...
		if ($in == 'ферма') {
				$opt = ['disable_mentions' => 1, 'dont_parse_links' => 1];
				$txt = '💬 Чтобы добывать ирис-коины перейдите в пост vk.com/wall-174105461_35135 и введите команду "ферма"';
				$vk->chatMessage($chatId, $txt, $opt);
				return;
		}
        ...
        if ((int)@$user["online"] > 0) {
            ...
            $msg.="$cnt. [id$id|$name] $smile $fr\n"; }
				...
				if (isset($userbot['mtoken']) && @$userbot['mtoken']!='') {
				$opt=['disable_mentions' => 1, 'dont_parse_links' => 1, 'expire_ttl' => 360]; 
				$vk = new UbVkApi($userbot['mtoken']); }
